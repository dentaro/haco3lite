...
// extern uint8_t premapArray[16][20];
...
int RunHaco8Game::l_mset(lua_State* L){
...
int RunHaco8Game::l_mget(lua_State* L){
  ...
  lua_pushinteger(L, mapArray[cely][celx]);//配列はyを先にしている
  return 1;
}
...
uint8_t getMapSprNo2csv(File& wfr, int wy, int wx) 
{
  String line;
  uint8_t sprno = 32;
  int i = 0; // 行のインデックス

  while (wfr.available() && i < wx) {
    line = wfr.readStringUntil('\n');
    int j = 0; // 列のインデックス
    int startIndex = 0;
    ...
    if (!line.isEmpty()) {
      while (j < wy) {
        ...
        int currentToken = token.toInt();

        if (i == wx - 1 && j == wy - 1) {
          // 望んでいる行と列の位置に到達したら、その時点で値を取得してループを終了
          sprno = currentToken;
          return sprno;  // 関数を抜ける前にreturnすることで、Fileが自動的に閉じられないようにする
        }
...
int RunHaco8Game::l_map(lua_State* L){
  ...
  // int wx = lua_tointeger(L, 7);
  // int wy = lua_tointeger(L, 8);
  return 0;
}

int RunHaco8Game::l_tool(lua_State* L){
  RunHaco8Game* self = (RunHaco8Game*)lua_touserdata(L, lua_upvalueindex(1));
...
//  while ( i < BSTARBUFNUM ) {
  for(int i =0; i< BSTARBUFNUM ;i++){
    ...
    // tft.drawPixel(64, 60, gethaco3Col(8)); 
  return 0;
}
...
int RunHaco8Game::l_print(lua_State* L){
  ...
  if(cn2 != NULL){
    self->col2[0] = self->clist[cn2][0]; // 5bit
    self->col2[1] = self->clist[cn2][1]; // 6bit
    ...
    //   if(j % numCorners == 0) j -= numCorners;//一周したらリセット
      ...
      if( i < numVertices - numCorners){//赤経をひく
        if( 0 == i % numCorners){//0度
          self->line3D(pg[i], pg[k], gethaco3Col(8));
        }
        else{
          self->line3D(pg[i], pg[k], gethaco3Col(1));
        }
      }
      ...
      if( i < numVertices - 1){//赤緯をひく
        ...
        // if(i <= numCorners-1){
          self->line3D(pg[i], pg[j], gethaco3Col(8));//0度
        }else{
          self->line3D(pg[i], pg[j], gethaco3Col(1));
        }
      }
...
int RunHaco8Game::l_spmap(lua_State* L) {
  ...
  int roiH = lua_tointeger(L, 7);
  int roiW = lua_tointeger(L, 8);
...
//１ピクセルずつ走査して描画していく
  ...
  for (int wx = 0; wx < w; wx++) {
    ...
    for (int wy = 0; wy < h; wy++) {
      ...
      if (transformedX < ((roix + roiH) << 3) && transformedY < ((roiy + roiW) << 3) && transformedX >= (roix << 3) && transformedY >= (roiy << 3)) {
