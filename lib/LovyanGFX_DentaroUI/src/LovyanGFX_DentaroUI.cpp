...
void LovyanGFX_DentaroUI::begin(LGFX &_lcd)
{
  ...
  //   m_url = "/tokyo/15/29094/"+String(12902 + i)+".png";

  //   this-> g_basic_sprite_list[i].setPsram(USE_PSRAM);
  //   // this-> g_basic_sprite_list[i].setPsram(false);//UNUSE_PSRAM
  //   this-> g_basic_sprite_list[i].setColorDepth(TILE_COL_DEPTH);//子スプライトの色深度
  //   this-> g_basic_sprite_list[i].createSprite(256, 256);//子スプライトメモリ確保
  //   // this-> g_basic_sprite_list[i].setColorDepth(TILE_CHILD_COL_DEPTH);//子スプライトの色深度

  //   this-> g_basic_sprite_list[i].setPaletteGrayscale();
  //   this-> g_basic_sprite_list[i].drawPngFile(SD, m_url,
  //                                 0, 0,
  //                                 256, 256,
  //                                 0, 0, 1.0, 1.0,
  //                                 datum_t::top_left);
  ...
  if (charMode == CHAR_3_BYTE)
  {
    ...
    if (str != "ま" || planeStr != "な")
    {
      if (_transID == 1 || _transID == 2)
      {
        ...
        if (i < HENKAN_NUM && getHenkanChar(i, _transID).c_str() != NULL)
          str = getHenkanChar(i, _transID).c_str();
        else
          str = "無";
      }
    }
  ...
  int _touchZoom, int _eventNo, int _xy_mode, int _uiContainerID)
{
  ...
  {
    panel->label = String(gPanelID);
    Serial.println("BTN_" + String(gPanelID) + "=[" + String(gPanelID) + "]" + String(gBtnID) + "~");
  }
  else if (_btn_mode == TOUCH_SLIDER_MODE)
  {
    panel->label = String(gPanelID);
    Serial.println("SLIDER_" + String(gPanelID) + "=[" + String(gPanelID) + "]" + String(gBtnID) + "~");
  }
  else if (_btn_mode == TOUCH_FLICK_MODE)
  {
    panel->label = String(gPanelID);
    Serial.println("FLICK_" + String(gPanelID) + "=[" + String(gPanelID) + "]" + String(gBtnID) + "~");
  }
...
void LovyanGFX_DentaroUI::deletePanels(int _uiContainerID, size_t PRESET_BTN_NUM) {
  if(_uiContainerID==1){
    ...
    gBtnID = PRESET_BTN_NUM;//再設定のため、ボタンIDを初期化
...
String LovyanGFX_DentaroUI::getFlickChar(int _uiId, int _btnID){
  String c = uis[_uiId]->panels[3]->touchBtns[_btnID-20]->namelabel;
  return c;
}
...
void LovyanGFX_DentaroUI::readFlickDataline(int _uiId, int _btnID) {
  int flickDatalineNo = flickShiftNo * 12 + _btnID - 32;//プリセットボタンの数32個を引いて0~12の値にする
    ...
    if (getEvent() == uis[_uiId]->panels[0]->eventNo || uis[_uiId]->panels[0]->eventNo == MULTI_EVENT)
    {
      ...
      for (int i = _id; i < _id + _btnNum; i++)
      {
        ...
        if (selectBtnID == i)
          ...
          uis[_uiId]->panels[i]->setSelectBtnF(false);
          uis[_uiId]->panels[i]->btnDraw(_lcd, 0, 0, 38, 48); // ボタンエリアの半径
        // panels[i]->btnDraw(_uiSprite, uis[_uiId]->panels[0]->x, uis[_uiId]->panels[0]->y); //スプライト経由で描画する
      }
    }
...
void LovyanGFX_DentaroUI::setSliderVal(int _uiId, int _btnNo, float _x, float _y)
{
  int _btnID = uis[_uiId]->panels[0]->b_sNo + _btnNo;
